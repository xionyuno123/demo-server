...
#endif
#define UNKOWN_IP_ADDR 0xffffffff
static uint32_t ip_addr_parse(const char * s){
	char * lim=NULL;
	uint32_t ret=0,ipaddr=0;
	ret=strtoul(s,&lim,10);
	if(ret>=255||lim==NULL || *lim++!='.'){
		perror("unkown ip addr\n");
		return UNKOWN_IP_ADDR;
	}
	ipaddr+=ret<<24;
	s=lim;
	ret=strtoul(s,&lim,10);
...
static int init_streams_pkt_header(char* config_filename){
	FILE * config_file;
	config_file=fopen(config_filename,"r");
	if (config_file == NULL){
		perror("Failed to open stream config file\n");
		return -1;
	}
	json_error_t error;
	json_t * arr=json_loadf(config_file,JSON_DECODE_ANY|JSON_DISABLE_EOF_CHECK,&error);
	if(arr == NULL) {
		perror("unformated configuration file\n");
		return -1;
	}
	size_t len=json_array_size(arr);
	if (len==0){
		perror("Check the format of config file\n");
		return -1;
	}

	int sm_id=0;
	unsigned int portlist[RTE_MAX_ETHPORTS],nb_pl=0;
	for (sm_id=0;sm_id<len;sm_id++){
		json_t *obj=json_array_get(arr,sm_id);
		json_t *val=json_object_get(obj,"port");
		if(val==NULL){
			perror("port id is neccesary for stream configuration\n");
			return -1;
		}
		int port=json_integer_value(val);
		if (port>=RTE_MAX_ETHPORTS || port >=nb_ports){
			perror("port number is too large\n");
			return -1;
		}

		if (port_rxq[port]==0&&port_txq[port]==0){
			portlist[nb_pl]=port;
			nb_pl++;
		}
		port_rxq[port]++;
		port_txq[port]++;
		streams_port[sm_id]=port;
		val=json_object_get(obj,"src_ip_addr");
		if (val!=NULL) { 
			uint32_t src_ip_addr=ip_addr_parse(json_string_value(val));
			if(src_ip_addr==UNKOWN_IP_ADDR){
				perror("unknown source ip addrress\n");
				return -1;
			}
			streams_src_ip_addr[sm_id]=src_ip_addr;
		}
		val=json_object_get(obj,"dst_ip_addr");
		if (val!=NULL){
			uint32_t dst_ip_addr=ip_addr_parse(json_string_value(val));
			if(dst_ip_addr==UNKOWN_IP_ADDR){
				perror("unknown destination ip addrress\n");
				return -1;
			}
			streams_dst_ip_addr[sm_id]=dst_ip_addr;
		}
		
		
		
		val=json_object_get(obj,"src_udp_port");
		if(val!=NULL)
		streams_src_port[sm_id]=json_integer_value(val);
		val=json_object_get(obj,"dst_udp_port");
		if(val!=NULL)
		streams_dst_port[sm_id]=json_integer_value(val);
		val=json_object_get(obj,"speed");
		if(val!=NULL)
		streams_speed[sm_id]=json_integer_value(val);

		val=json_object_get(obj,"src_mac_addr");
		if(val!=NULL)
		if (rte_ether_unformat_addr(json_string_value(val),&streams_src_mac_addr[sm_id])<0){
			perror("unknown source mac addrress\n");
			return -1;
		}

		val=json_object_get(obj,"dst_mac_addr");
		if(val!=NULL)
		if (rte_ether_unformat_addr(json_string_value(val),&streams_dst_mac_addr[sm_id])<0){
			perror("unknown destination mac addrress\n");
			return -1;
		}

		val=json_object_get(obj,"retry_enabled");
		if(val!=NULL){
			retry_enabled=json_boolean_value(val)?1:0;
		}

		val=json_object_get(obj,"packet_size");
		if(val!=NULL){
			streams_packet_size[sm_id]=json_integer_value(val);
		}

		printf("port: %d src_ip_addr: %u, dst_ip_addr:%u, src_port: %d, dst_port:%d, speed: %d, dst_mac_addr:%u, src_mac_addr:%u \n",
		port,
		streams_src_ip_addr[sm_id],
		streams_dst_ip_addr[sm_id],
		streams_src_port[sm_id],
		streams_dst_port[sm_id],
		streams_speed[sm_id],
		streams_dst_mac_addr[sm_id],
		streams_src_mac_addr[sm_id]
		);
	}
	set_fwd_ports_list(&portlist,nb_pl);
	return 0;
}
...
#define SHORTOPTS ""
	...
	while ((opt = getopt_long(argc, argvopt, SHORTOPTS "ah",
		...
		case 0: /*long options */
			...
			if (!strcmp(lgopts[opt_idx].name, "shunting-configfile")) {
				if (init_streams_pkt_header(optarg) != 0)
					rte_exit(EXIT_FAILURE,
						 "parse shunting-configfile error\n");
				split_streams_enabled=true;
				// switch to shunting-client-engine
				set_pkt_forwarding_mode("shunting");
			}
			if (!strcmp(lgopts[opt_idx].name, "httpd")) {
