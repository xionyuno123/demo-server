...
static void 
set_def_stream_ip_addrs(void){
	uint16_t tx_udp_src_port = 9;
    uint16_t tx_udp_dst_port = 9;
	uint32_t tx_ip_src_addr = (198U << 24) | (18 << 16) | (0 << 8) | 1;
    uint32_t tx_ip_dst_addr = (198U << 24) | (18 << 16) | (0 << 8) | 2;
	streamid_t i=0;
	for(i;i<RTE_MAX_STREAMS;i++){
		streams_dst_ip_addr[i]=tx_ip_dst_addr;
		streams_src_ip_addr[i]=tx_ip_src_addr;
		streams_dst_port[i]=tx_udp_dst_port;
		streams_src_port[i]=tx_udp_src_port;
		streams_packet_size[i]=64;
	}
}
...
init_fwd_streams(void)
{
	...
	streamid_t sm_id, nb_fwd_streams_new=0;
	queueid_t q;
	...
	RTE_ETH_FOREACH_DEV(pid) {
		...
		int rxq=port_rxq[pid];
		int txq=port_txq[pid];

		nb_fwd_streams_new+=RTE_MIN(rxq,txq);

		if (rxq > port->dev_info.max_rx_queues) {
			printf("Fail: nb_rxq(%d) is greater than "
				"max_rx_queues(%d)\n", rxq,
				port->dev_info.max_rx_queues);
			return -1;
		}
		if (txq > port->dev_info.max_tx_queues) {
	...
	if(!split_streams_enabled)
	nb_fwd_streams_new = (streamid_t)(nb_ports * q);

	if (nb_fwd_streams_new == 0) {
		printf("Fail: fwd streams can not be zero\n");
		return -1;	
	}
	...
	printf("[Streams] nb_fwd_streams=%d \n",nb_fwd_streams);
	if (nb_fwd_streams) {
...
void clear_stream_stats_all(){
	uint16_t sm_id=0;
	for(sm_id=0;sm_id<nb_fwd_streams;sm_id++){
		clear_stream_stats(sm_id);
	}
}

void clear_stream_stats(uint16_t sm_id){
	if(sm_id<nb_fwd_streams){
		printf("clear stream %d ....\n", sm_id);
	}
}
...
void show_stream_stats_all(){
	uint16_t sm_id=0;
	for(sm_id=0;sm_id<nb_fwd_streams;sm_id++){
		show_stream_stats(sm_id);
	}
}

void show_stream_stats(uint16_t sm_id){
	if(sm_id<nb_fwd_streams){
		printf("show stream %d ....\n", sm_id);
	}
}
...
run_pkt_fwd_on_lcore(struct fwd_lcore *fc, packet_fwd_t pkt_fwd)
{
	...
	uint64_t tics_datum[RTE_MAX_STREAMS];
	...
	nb_fs = fc->stream_nb;
	tics_space=&streams_tics_space[fc->stream_idx];
	speeds=&streams_speed[fc->stream_idx];
	packet_sizes=&streams_packet_size[fc->stream_idx];
	for (sm_id = 0; sm_id < nb_fs; sm_id++){
		tics_datum[sm_id]=rte_rdtsc();
		if(speeds[sm_id]==0){
			tics_space[sm_id]=0;
			continue;
		}
		uint64_t speed=speeds[sm_id]; // bytes/s
		speed=speed*1000*1000/8;
		uint64_t burst_bytes=nb_pkt_per_burst*packet_sizes[sm_id];
		uint64_t cnt=speed/burst_bytes;
		tics_space[sm_id]=tics_per_1sec/cnt;
		printf("sp: %dMbit/s speed: %ld, tics_space[%ld]=%ld \n", speeds[sm_id],speed,sm_id,tics_space[sm_id]);
	}
	...
	do {
		for (sm_id = 0; sm_id < nb_fs; sm_id++)
		{
			...
			if(tics_current-tics_datum[sm_id] >= tics_space[sm_id])
			{
				tics_datum[sm_id]=tics_current;
				(*pkt_fwd)(fsm[sm_id]);
			}
...
start_port(portid_t pid)
{
	...
	portid_t pl[RTE_MAX_ETHPORTS];// 成功配置的port
	portid_t peer_pl[RTE_MAX_ETHPORTS];
	uint16_t cnt_pi = 0;
	uint16_t cfg_pi = 0; // 成功配置的port数量
	int peer_pi;
	queueid_t qi;
	struct rte_port *port;
	struct rte_eth_hairpin_cap cap;
	...
	RTE_ETH_FOREACH_DEV(pi) {
		...
		int rxq=nb_rxq,txq=nb_txq;
		if(port_txq[pi]&&split_streams_enabled)
		txq=port_txq[pi];
		if(port_rxq[pi]&&split_streams_enabled)
		rxq=port_rxq[pi];
		...
		if (port->need_reconfig > 0) {
			...
			// 移除掉dump callback function
			configure_rxtx_dump_callbacks(0);
			printf("Configuring Port %d (socket %u)\n", pi,
					port->socket_id);
			/* 
				hairpin 功能就是应用程序向这个队列发送的包最终会被发送到该应用程序，不经过wired cable。
				nb_hairpinq 
			 */
			if (nb_hairpinq > 0 &&
			...
			/* configure port */
			diag = rte_eth_dev_configure(pi, rxq + nb_hairpinq,
						     txq + nb_hairpinq,
						     &(port->dev_conf));
			printf("[Streams] rte_eth_dev_configure(%d,%d,%d)\n",pi,rxq+nb_hairpinq,txq+nb_hairpinq);
			if (diag != 0) {
				if (rte_atomic16_cmpset(&(port->port_status),
...
static void
rxtx_port_config(struct rte_port *port,portid_t pid)
{
	uint16_t qid;
	uint64_t offloads;

	int rxq=nb_rxq, txq=nb_txq;
	if (port_rxq[pid]&&split_streams_enabled)
	rxq=port_rxq[pid];
	if(port_txq[pid]&&split_streams_enabled)
	txq=port_txq[pid];

	for (qid = 0; qid < rxq; qid++) {
...
init_port_config(void)
{
	...
	RTE_ETH_FOREACH_DEV(pid) {
		...
		int rxq=nb_rxq;
		if (port_rxq[pid]!=0&&split_streams_enabled)
		rxq=port_rxq[pid];
